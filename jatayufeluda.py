...
def align(wtype,len1,mutation_sequence,position,variation):
    ...
    filename = path1 + uuid.uuid4().hex + ".fasta"
    ...
    #df['SEQ'] = wt_seq
     ...
     cmdArgs3=['bedtools', 'intersect', '-a', str(filename_bed),'-b', 'gencode.v32.annotation_genes_filtered.gtf' , '-wb'   ]
     call(cmdArgs3,stdout=open(filename_bed_gene,'w'))
     cmdArgs4=[ '/usr/bin/cut',  '-f' , '1-6,9', filename_bed_gene]
     ...
     df3.columns=['Chr','Start','End', 'Strand'  ,'WT-Seq', 'Disease_Seq',  'Gene','Start_sgRNA','End_sgRNA','sgrna','sgRNA','oligo']
     print (df3['oligo'][0])
    ...
    #df3['sgrna_seq']= df3.apply(lambda row : sgrna_des(row['sgrna']), axis = 1)
     ...
     df3['Coordinate']= df3["Chr"].astype(str) +":"+ df3["Start_sgRNA"].astype(str)   + "-" + df3["End_sgRNA"].astype(str)
     df3=df3.drop([ 'Chr', 'Start_sgRNA', 'End_sgRNA', 'Start','End', 'Strand'  ,'WT-Seq','Disease_Seq','sgrna','oligo'],axis=1   )  
